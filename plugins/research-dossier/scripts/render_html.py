    ...
    while index < len(lines):
        ...
        if re.match(r"^[-*]\s+", line):
            ...
            while index < len(lines) and re.match(r"^[-*]\s+", lines[index]):
                ...
                index += 1
            body.append(f'<ul>{"".join(items)}</ul>{copy_button("\n".join(raw_items))}')
            continue
        paragraph = [line]
        index += 1
        while index < len(lines) and lines[index].strip() and not re.match(r"^(#{1,3})\s+|^\||^>\s|^[-*]\s+", lines[index]):
            ...
            index += 1
        text = "\n".join(paragraph)
        body.append(f'<div class="content-block"><p>{inline(text)}</p>{copy_button(text)}</div>')
...
<title>Исследовательское досье</title><style>{CSS}</style></head><body>
