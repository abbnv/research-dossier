...
Research Dossier превращает тему, идею, транскрипт, заметки, URL или запись созвона в локальное исследовательское досье для будущего видео. Это топливо для ресерча, а не готовый сценарий: отчёт объединяет доказательства, концепции, кейсы, цитаты, медиарекомендации, метафоры и десять смелых творческих направлений. Надёжность материала показывается источниками, точностью формулировок, ограничениями и отдельным внутренним журналом проверки, без шумных статусных тегов в итоговом тексте.
...
5. Разделяет доказательные находки, редакторские интерпретации, творческие гипотезы и короткие вымышленные иллюстрации структурой отчёта и формулировками.
...
Если marketplace уже добавлен, обновите локальный каталог и переустановите плагин:
...
Для локальной разработки используйте инструкции [docs/updating.md](docs/updating.md). После обновления начните новую задачу Codex, чтобы она загрузила новую версию навыка. Версия плагина указана в [plugins/research-dossier/.codex-plugin/plugin.json](plugins/research-dossier/.codex-plugin/plugin.json).
